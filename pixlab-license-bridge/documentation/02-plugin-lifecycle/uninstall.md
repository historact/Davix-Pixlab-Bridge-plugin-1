...
- Removal is gated by the `dsb_delete_on_uninstall` option. When truthy:
  ...
  - Deletes options: `dsb_settings`, `dsb_product_plans`, `dsb_plan_products`, `dsb_plan_sync`, resync/node-poll/purge lock and status options, `pixlab_license_db_version` (and the legacy db version option), and uninstall flag itself.
