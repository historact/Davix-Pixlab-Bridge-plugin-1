...
- Log directory: Pixlab License Bridge Logs stored in `wp-content/uploads/pixlab-license-bridge-logs/`; created with index.php and .htaccess to block listing.
...
```nginx
location ^~ /wp-content/pixlab-license-bridge-logs/ { deny all; }
location ^~ /wp-content/uploads/pixlab-license-bridge-logs/ { deny all; }
location ^~ /wp-content/uploads/pixlab-license-bridge-logs/cron/ { deny all; }
```
